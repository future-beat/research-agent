# Requirements: v1.2 — Nothing uncovered  (ARCHIVED)

**Status:** ✅ ALL NINE COMPLETE — archived 2026-08-17
**Coverage:** 9/9, no orphans. Seven at milestone open; `REQ-classifier-model` and
`REQ-demo-shows-progress` were both defined mid-milestone, the first from what the paid
record run measured and the second from a live user report.
**Audit:** [`v1.2-MILESTONE-AUDIT.md`](../v1.2-MILESTONE-AUDIT.md) — `passed`

> **One requirement was renegotiated with evidence rather than met as written.**
> `REQ-forty-recorded-answers` asked for all forty cases recorded AND for committed
> fixtures to be grader-and-judge approved. The paid run measured those clauses as
> incompatible on the real pipeline. The ratified criterion became *recorded or documented
> refusal*, and the literal original wording was never met — stated here so the archive
> inherits it rather than implying forty.

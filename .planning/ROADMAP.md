...
| 15. Answer-quality evals | v1.1 | 2/6 | In Progress | Waves 1–2 done. Wave 1: the recorder seam (`run_case(capture_state=True)`) and `evals/fixtures.py` — schema_version 1, a models MAP rather than a flat model string, refusal of any recording whose own grades failed, total load-time validation. Wave 2: the grading vocabulary for recorded answers — grounding (risky-token containment, with `1M`/`1 million`/`$2,000` normalisation), coverage, structure, refusal honesty and per-case pins, each with a synthetic state it passes AND one it fails, each stating what it cannot catch, enforced by a test over the registries. 23 mutations run: 22 red on the intended assertion, the 23rd green and repaired (the refusal grader's "the question is a source" rule was ungated). Thresholds recorded as unvalidated: against the twelve scripted reports the 200-char floor passes 0/12 and coverage-at-0.4 passes 6/12 — wave 6's calibration recording is where they meet real output. Nothing recorded, nothing wired into a run, no spend. Suites 615/65 plain, 679/1 armed, offline evals 12/12 keyless. Branch `gsd/phase-15-answer-quality-evals`. |

...
| 14. Real cost accounting | v1.1 | 1/3 | In Progress | Wave 1 in (14-01): `discount × geo` applies at `CallUsage.cost_usd` and nowhere else in `src/` (grep baseline 0 files → exactly 1), with the geo *rate* env-configured and its *applicability* read off each response's `usage.inference_geo` — `""`/`"global"` unmultiplied, `"us"` at the rate, anything else raising into `pricing_unknown` through `record()`'s untouched except clause. Non-positive or unparseable factors fall back, so a `COST_DISCOUNT_FACTOR=0` typo cannot cost every run at $0.00 and disarm the caps. Four mutations red by the intended assertion; both `-k` selectors confirmed to collect first. Suites 539/63 plain, 601/1 armed (+10 both arms, zero new skips), `ruff` clean. README's "no enterprise discounts or `inference_geo` multiplier" is now false and is 14-03's to rewrite. Branch `gsd/phase-14-real-cost-accounting`; nothing pushed, nothing deployed. |

...
| 15. Answer-quality evals | v1.1 | 3/6 | In Progress | Waves 1–3 done. Wave 1: the recorder seam (`run_case(capture_state=True)`) and `evals/fixtures.py` — schema_version 1, a models MAP rather than a flat model string, refusal of any recording whose own grades failed, total load-time validation. Wave 2: the grading vocabulary for recorded answers — grounding (risky-token containment, with `1M`/`1 million`/`$2,000` normalisation), coverage, structure, refusal honesty and per-case pins, each with a synthetic state it passes AND one it fails, each stating what it cannot catch, enforced by a test over the registries. Thresholds recorded as unvalidated: against the twelve scripted reports the 200-char floor passes 0/12 and coverage-at-0.4 passes 6/12 — wave 6's calibration recording is where they meet real output. **Wave 3: both are now wired into the run.** `python -m evals` replays whatever fixtures are committed, keylessly, with the CI command and `ANTHROPIC_API_KEY: ""` unchanged; the replay leg is **all-must-pass at the exit code** while the behavioural leg stays rate-governed at 0.9, because `summarise`'s `ok` is a pass rate and nothing else (12 green + 1 red replay = 92.3% ⇒ a rate-only verdict exits 0 and every hard gate is decorative). Broken and orphaned fixtures are loud reds, not tracebacks; zero fixtures is still green; the caveat now prints the recording's date/model/sha/age and says the answers graded are "not what the current model would say". 18 mutations, 18 red. **Nothing recorded, no spend** — the run still prints the original caveat and exits 0 on 12/12. Suites 630/65 plain, 694/1 armed. Branch `gsd/phase-15-answer-quality-evals`. |

...
| 15. Answer-quality evals | v1.1 | 4/6 | In Progress | Waves 1–4 done. Wave 1: the recorder seam (`run_case(capture_state=True)`) and `evals/fixtures.py` — schema_version 1, a models MAP rather than a flat model string, refusal of any recording whose own grades failed, total load-time validation. Wave 2: the grading vocabulary for recorded answers — grounding (risky-token containment, with `1M`/`1 million`/`$2,000` normalisation), coverage, structure, refusal honesty and per-case pins, each with a synthetic state it passes AND one it fails, each stating what it cannot catch, enforced by a test over the registries. Wave 3: both wired into the run — `python -m evals` replays whatever fixtures are committed, keylessly, with the CI command and `ANTHROPIC_API_KEY: ""` unchanged; the replay leg is **all-must-pass at the exit code** while the behavioural leg stays rate-governed at 0.9 (12 green + 1 red replay = 92.3% ⇒ a rate-only verdict exits 0 and every hard gate is decorative); broken and orphaned fixtures are loud reds; the caveat prints the recording's date/model/sha/age and says the answers graded are "not what the current model would say". **Wave 4: the dataset is a benchmark.** GOLDEN 12 → 40 across the locked taxonomy, every case stating what it exists to catch; the `no_prior_research` stop gets its first golden case (measured baseline: zero) via `Followup.expect_forced_stop` + `grade_followup_forced_stop`; two adversarial cases seed a poisoned note into the case's OWN store through the new `Case.seeded_notes` seam, under a heavy-overlap authoring rule that is measured (7-word overlap recalls 60/60 hash seeds, 1-word 17/60) and pinned. Seven `test_dataset_taxonomy_*` property pins count the data rather than a list beside it. **The 28 new reports are written to report length**, so wave 6 calibrates against a corpus instead of stubs: the existing 12 clear the 200-char floor 0/12 and coverage-at-0.4 6/12, the new 28 clear both 27/28 — no threshold moved. 18 mutations, 17 red + 1 honest green (a floor with one case of slack; draining two reds it). Four cases tagged "Phase 17" so its flip stays a measurement rather than an edit to history. **Nothing recorded, no spend** — the run still prints the original caveat and exits 0, now on 40/40. Suites 645/65 plain, 709/1 armed. Branch `gsd/phase-15-answer-quality-evals`. |

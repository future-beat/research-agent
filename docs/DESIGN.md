...
**Prices are effective-dated, because one of them is time-boxed.** Claude Sonnet 5 is on introductory pricing of $2/$10 per MTok through 2026-08-31, and the standard $3/$15 rate takes over from 2026-09-01. A single hardcoded rate would keep reporting confident numbers that are a third too low from that date on — the worst kind of wrong, because nothing fails. `price_for()` resolves the rate for a date, a test pins both windows, and `/pricing` exposes what accounting is using today so a step in the cost graph has a visible cause. Cache-rate constants are asserted to be the documented 1.25× and 0.1× of base input rather than trusted as typed-in numbers.
